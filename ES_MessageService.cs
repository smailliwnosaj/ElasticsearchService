...
{
    public class ES_MessageService : ES_DataService , IElesticsearchService<ES_Message, ES_QueryMessages>
    {
        public ES_MessageService() : base(ES_ClusterName.messages)
        {
            initialize();
        }

        public ES_MessageService(ES_Config config) : base(config, ES_ClusterName.messages)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<ES_Message>> GetByQueryAsync(ES_QueryMessages query, int tenantId)
        {
            var message = await Task.Run(() => GetByQuery(query, tenantId));
            return message;
        }

        public List<ES_Message> GetByQuery(ES_QueryMessages query, int tenantId)
        {
            var json = new JavaScriptSerializer().Serialize(query);

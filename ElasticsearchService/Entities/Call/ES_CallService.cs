...
namespace ES
{
    public class ES_CallService : Base_Classes.ES_DataService, Interfaces.IElesticsearchService<ES_Call, ViewModels.ES_Query>
    {
        public ES_CallService() : base(Base_Classes.ES_ClusterName.calls)
        {
            initialize();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<ES_Call>> GetByQueryAsync(ViewModels.ES_Query query, int tenantId)
        {
            var call = await Task.Run(() => GetByQuery(query, tenantId));
            return call;
        }

        public List<ES_Call> GetByQuery(ViewModels.ES_Query query, int tenantId)
        {
            var json = new JavaScriptSerializer().Serialize(query);
